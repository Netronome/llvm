...
// CHECK: 1d 10 0b 00 01 00 00 00 	if w0 == w1 goto +11
// CHECK: 5d 43 0a 00 01 00 00 00 	if w3 != w4 goto +10
...
// CHECK: 2d 21 09 00 01 00 00 00 	if w1 > w2 goto +9
// CHECK: 3d 32 08 00 01 00 00 00 	if w2 >= w3 goto +8
// CHECK: 6d 54 07 00 01 00 00 00 	if w4 s> w5 goto +7
// CHECK: 7d 65 06 00 01 00 00 00 	if w5 s>= w6 goto +6
...
// CHECK: ad 76 05 00 01 00 00 00 	if w6 < w7 goto +5
// CHECK: bd 87 04 00 01 00 00 00 	if w7 <= w8 goto +4
// CHECK: cd 98 03 00 01 00 00 00 	if w8 s< w9 goto +3
// CHECK: dd a9 02 00 01 00 00 00 	if w9 s<= w10 goto +2

  if w0 == 0 goto Llabel0           // BPF_JEQ  | BPF_K
  if w3 != -1 goto Llabel0          // BPF_JNE  | BPF_K
// CHECK: 15 10 01 00 00 00 00 00 	if w0 == 0 goto +1
// CHECK: 55 13 00 00 ff ff ff ff 	if w3 != -1 goto +0
...
// CHECK: 25 11 ff ff 40 00 00 00 	if w1 > 64 goto -1
// CHECK: 35 12 fe ff ff ff ff ff 	if w2 >= -1 goto -2
// CHECK: 65 14 fd ff ff ff ff ff 	if w4 s> -1 goto -3
// CHECK: 75 15 fc ff ff ff ff 7f 	if w5 s>= 2147483647 goto -4
...
// CHECK: a5 16 fb ff ff 00 00 00 	if w6 < 255 goto -5
// CHECK: b5 17 fa ff ff ff 00 00 	if w7 <= 65535 goto -6
// CHECK: c5 18 f9 ff 00 00 00 00 	if w8 s< 0 goto -7
// CHECK: d5 19 f8 ff ff ff ff ff 	if w9 s<= -1 goto -8
